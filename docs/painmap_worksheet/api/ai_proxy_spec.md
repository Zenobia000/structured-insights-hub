...
> **真相源**：`product/data_model.md` + `references/ai_prompt_library.md`（後者於 M1 後續批次建立）
...
完整 prompt 內文存在 `references/ai_prompt_library.md`（後續批次建立）。本檔僅列對應關係：
...
| 5 | `p5_triz_contradiction` | 從 6 種 TRIZ 矛盾建議使用者最可能的 1-2 個 | Claude | HIGH |
...
**Response 200**（stream=false）：`data: { prompt_run_id, provider, model, raw_response, parsed: { q1..q8 / 各 prompt schema }, no_solution_check_passed, tokens_used: { prompt, completion }, cost_usd }`
...
**Status codes**：200 / 400 INVALID_PROMPT_ID / 401 / 422 SOLUTION_MODE_DETECTED（自動重試 1 次後仍違規） / 429 / 503 LLM_PROVIDER_DOWN。
...
2. 載入 prompt template by prompt_id（從內建模板表）
3. 注入 system prompt（教學模式 guard，見 3.3）
4. 變數插值（伺服器端，不信任前端送的變數）
5. 呼叫 LLM provider API（OpenAI / Anthropic / Google）
6. 收到回應 → 跑 anti-solution mode 檢查（regex + LLM-judge）
7. 若違規且未重試 → 注入 stricter guard 再跑一次
8. 若仍違規 → 回 422 SOLUTION_MODE_DETECTED
9. parse 結構化欄位（依 prompt schema）
10. 回傳給前端
11. （opt-in 才）寫入 audit log
```

---

### 3.3 教學模式專屬 system prompt（強制注入）

每次後端呼叫 LLM 時，**永遠**先注入下列 system prompt（使用者前端的 prompt 不可覆蓋）：

```
你正在協助使用者進行 PainMap 痛點教學流程。請嚴格遵守以下原則：
...
若使用者讀完仍覺得 AI 在推方案 → 點「這份回應在推方案，幫我重跑」按鈕 → 觸發 strict 模式重跑。

---

### 3.5 API call payload（各家適配）

統一參數：`temperature=0.7`、`max_tokens=4000`、教學模式 system prompt（3.3）強制注入。
