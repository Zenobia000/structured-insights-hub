...
3. **過關條件透明**：兩個空格具體 + 確認能回答 AI 列的問題
4. **失敗回退**：過不了 → 退回卡 1（找主人翁再聊一次）
...
8. **exit_gate_footer** — 過關條件 + 「儲存並進入卡 4」
...
- `body` (Body MD): "AI 列的「需要再問清楚」如果你**能回答**或**已預約找主人翁問**，就勾選確認。如果都答不出來 → 退回卡 1，去找主人翁再聊一次。"
...
| `confirmed` 未勾選 | 「請確認你能回答 AI 列的問題（或已預約找主人翁問）。如果都答不出來 → 退回卡 1。」 |
...
- [ ] retreat_action 「答不出來退回卡 1」link 正確導向，LocalStorage 卡 3 資料保留
