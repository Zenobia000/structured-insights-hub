...
> 對應 `painmap_worksheet/product/data_model.md` 的 PainCard schema 與 `references/exit_gates_matrix.md` 的過關條件。

---

## 1. 設計目標

1. **線性主流程，但允許回看** — 9 張卡片是有序流程，使用者可以「過關後回去檢視」，但不能「跳關」未完成的卡片
2. **永遠知道自己在哪** — 每一步都清楚告訴使用者目前在第幾張卡片、距離終點還有多遠
3. **失敗有方向** — 過不了關時，回退路徑必須是「友善建議」而非「失敗懲罰」
...
├─ /learn/worksheet/05                    ← 卡 5：兩件事不能同時要（TRIZ 提案）
...
| 2 | `/learn/worksheet/02` | 三個有名字的人 | form_card | noindex | 卡 1 過關 |
| 3 | `/learn/worksheet/03` | 卡關公式 | form_card_ai | noindex | 卡 2 過關 |
| 4 | `/learn/worksheet/04` | 現在怎麼解 | form_card_ai | noindex | 卡 3 過關 |
| 5 | `/learn/worksheet/05` | 矛盾選擇 | form_card_ai | noindex | 卡 4 過關 |
| 6 | `/learn/worksheet/06` | AI 證據蒐集 | form_card_ai | noindex | 卡 5 過關 |
| 7 | `/learn/worksheet/07` | 自己先猜 + 讀 AI | form_card_ai | noindex | 卡 6 過關 |
| 8 | `/learn/worksheet/08` | 訪談規劃 | form_card_ai | noindex | 卡 7 過關 |
| 9 | `/learn/worksheet/09` | 真假判斷 | form_card | noindex | 卡 8 過關 |
| 10 | `/learn/worksheet/result` | 痛點身份證匯出 | export_view | noindex | 卡 9 過關 |
...
每張卡片底部「下一步」按鈕（`exit_gate_check.md` 元件）按下後：

| 當前 | 過關後目標 | 失敗時 |
| :--- | :--- | :--- |
| `/01` | `/02` | 不導向，顯示「還缺什麼」訊息 |
| `/02` | `/03` | 不導向 + tooltip |
| `/03` | `/04` | 不導向 + tooltip |
| `/04` | `/05` | 不導向 + tooltip |
| `/05` | `/06` | 不導向 + tooltip |
| `/06` | `/07` | 不導向 + tooltip（含 AI 反 solution mode 警告） |
| `/07` | `/08` | 不導向 + tooltip |
| `/08` | `/09` | 不導向 + tooltip |
| `/09` | `/result` | 不導向 + tooltip |
| `/result` | （終點） | — |

### 7.2 失敗時的回退路徑

依據 `references/exit_gates_matrix.md`，失敗時不強制路由，而是「建議」：

| 卡片 | 失敗類型 | 建議回退 | 文案範例 |
| :--- | :--- | :--- | :--- |
| 卡 2 | 還沒接觸真人 | 回卡 1 | "你還沒接觸真人，先去找一個真人聊聊再回來吧。要回到卡 1 嗎？" |
| 卡 3 | 句子不夠具體 | 回卡 1 | "句子裡的兩個空格還太抽象，你可能要再去問主人翁一次。" |
| 卡 4 | 不滿意理由不足 | 回卡 1 | "他現在用的方法說不出 3 個不滿，可能他沒在花時間解。" |
| 卡 5 | 選了多項或都不像 | 回卡 3 | "矛盾沒拆乾淨。回到卡 3 把卡關公式再講清楚。" |
...
| 版本 | 日期 | 變更 |
| :--- | :--- | :--- |
| 1.0 | 2026-05-01 | 首版；定義 11 頁路由、stepper 行為、deep link 規則、SEO 策略 |
