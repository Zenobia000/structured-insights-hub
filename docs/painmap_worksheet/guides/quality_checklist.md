...
> **配套文件**：`implementation_guide.md`

---

## 0. 為什麼需要這份清單

PainMap Worksheet 同時要做兩件互相張力的事：(1) 訓練判斷力（需要進度感、成就感 → 容易偷渡黑帽）；(2) 嚴禁分數 / 排行榜 / 徽章（brand 鐵律）。這個張力導致 reviewer 容易「不小心放行」違規。本 checklist 把禁令、必檢項、灰色地帶判定一次列清楚。
...
- [ ] **沒有任何分數 UI**：不出現「23/25」「85%」「85 分」「N / 100」「N / 25」（schema 內無 score 欄位）
...
- [ ] 卡 5 使用者用自己的話寫 side_a / side_b / sacrificed_reason（無 radio group / 無分類學標籤）
...
- [ ] **Card 5 contradiction**：`side_a`、`side_b`、`sacrificed` (a|b)、`sacrificed_reason`
...
- [ ] **Card 9 verdict**：`judgment`、`reason_100w`、`most_confident_evidence`、`least_confident`、`next_action`（無 score 欄位）
- [ ] **exported**：`exported_at`、`formats[]`、`last_review_at`

### 3.2 Validation Rules 全部實作
...
- [ ] **R2 內容**：
  ...
  - [ ] R2.5 卡 5 `sacrificed_reason` 非空且 ≥ 1 句完整描述
  - [ ] R2.6 anti-solution mode regex 偵測（`ai_proxy_spec.md` §2.3）
  - [ ] R2.7 anti-taxonomy regex 偵測：卡 5 AI 回應不可含任何編號或分類學標籤
- [ ] **R3 跨欄位一致性**：
...
- [ ] **卡 5**：`side_a` / `side_b` 非空 + `sacrificed ∈ {'a', 'b'}` + `sacrificed_reason` 非空
...
- [ ] **卡 9**：`judgment` 已選 + `reason_100w.length >= 100` + `most_confident_evidence` 非空 + `least_confident` 非空 + `next_action` 已選
...
- [ ] 反思提示 / CTA 啟用判定函式為純函式；不可變更新（spread operator）
...
- [ ] Schema：PainCard 欄位與 data_model.md 一致；R1-R3 涉及項目已實作；schema 內無 score 欄位、無分類學欄位
- [ ] Functional：反思提示 / CTA 判定函式有單元測試；失敗訊息使用 brand voice；LocalStorage 寫入有 debounce；跨 session 復原驗證過
...
| 等級 | 定義 | 範例 |
| :--- | :--- | :--- |
| **CRITICAL** | 違反 brand 鐵律、schema 不一致、隱私洩漏、CTA 判定錯誤 | 出現分數 UI、`people.list` 接受 4 筆 |
| **HIGH** | 影響核心使用者旅程、A11y 違規、安全漏洞 | 提示文案焦慮、缺 ARIA、明文存 API key |
