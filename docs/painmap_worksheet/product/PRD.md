...
> **真相源**：`docs/workshop/painpoint_beginner_worksheet.md` v1.0、`docs/painmap_worksheet/product/data_model.md` v1.0
...
- Pain Quality Score (0-25) 僅限教學模式內部反思，**生產模式禁止輸出**（避免違反 Anti-Score Brand）
...
- **失敗情境**：如果工具用「點子分數」呈現結果，他會誤以為 24 分就是綠燈，跳過真人訪談直接寫 code
...
| 怕做錯 | 每張卡片有過關條件 + 失敗回退路徑（卡 2 fail → 回卡 1 等）|
...
| 評分系統誤用 | CRITICAL | 永久禁用分數 UI（生產模式 status only；教學模式 score 僅限內部反思）|
...
[卡 1] 抱怨原句          ← 你來填，AI 禁用
    ↓ exit gate
[卡 2] 三個有名字的人     ← 你來填，AI 禁用
    ↓ exit gate
[卡 3] 卡關公式          ← 你來填 + AI 校對
    ↓ exit gate
[卡 4] 現在怎麼解        ← 你來填 + AI 提案 5 個 workaround
    ↓ exit gate
[卡 5] TRIZ 矛盾選擇      ← 你來選 + AI 提案
    ↓ exit gate
[卡 6] AI 證據蒐集        ← AI 跑研究（8 題）
    ↓ exit gate
[卡 7] 自己先猜 + 讀 AI   ← 你先寫猜測 → AI 整理判斷表 → 對照差異
    ↓ exit gate
[卡 8] 真人訪談規劃       ← 你選對象 + AI 模擬訪談（熱身）
    ↓ exit gate
[卡 9] 真假判斷           ← 你來判，AI 禁用（核心交付物）
    ↓ exit gate
[卡 10] 痛點身份證        ← 整合匯出
```
...
| #2 Development & Accomplishment | 9 卡 stepper 進度、過關條件視覺化、痛點身份證 = 完成憑證 |
...
- [ ] LocalStorage 持久化（key: `painmap_worksheet:cards`）
- [ ] 過關條件機制（exit gate）：未通過不能進下一卡
- [ ] 失敗回退路徑：依 spec §6 規則
- [ ] 7 段內建 prompt（卡 3 / 4 / 5 / 6 / 7 第一輪 / 7 第二輪 / 8）一鍵複製
...
- [ ] 教學模式 / 生產模式切換開關（toggle）
- [ ] 卡 9「分數帶解讀」說明文字（強調「分數是鏡子，不是答案」）
...
| 卡片過關率 | 每張卡片 exit gate 通過率（單獨計算）| 卡 1–5 ≥ 70%；卡 6–8 ≥ 50%；卡 9 ≥ 80% |
| 跨 session 復原率 | 中斷後回來繼續填的比例 | ≥ 40% |
| 匯出觸發率 | 完成卡 9 → 觸發匯出的比例 | ≥ 70% |
| 失敗回退使用率 | 卡 X fail → 回到上一卡修正的比例 | ≥ 60%（健康訊號）|
| 平均完成時間 | 卡 1 開始到卡 9 完成的中位時間 | 第一次 60–90 分鐘；熟練後 30 分鐘 |
| 換題率 | 卡 X fail 後棄用此 PainCard 重新建立的比例 | ≤ 30%（過高代表入門門檻太高）|
...
| 數字 | 85/100 顯眼 | 0–25 score 僅限教學模式內部反思 |
...
- 繁體中文 (zh-Hant) 優先
- 技術術語可保留英文（API、JSON、UUID、JTBD、TRIZ、SECI、Octalysis、CTA 等）
...
| 使用者在卡 9 把分數當答案 | MEDIUM | HIGH | 卡 9 必須顯示「分數是鏡子不是答案」說明，並要求書面 100 字理由 |
