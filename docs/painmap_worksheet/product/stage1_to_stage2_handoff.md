# 階段一 → 階段二 銜接規格 (Stage 1 → Stage 2 Handoff)

> **版本**：v1.0 — 2026-05-01
> **配套文件**：`PRD.md`、`user_journey.md`、`motivation_design.md`、`data_model.md`
...
4. 哪些欄位該保留 / 該丟棄？
5. 教學模式（25 分 score）如何切換到生產模式（status only）？
6. 假痛點 / 待訪談 怎麼處理？
...
| `contradiction.triz_id` | `contradiction.triz_id` | 保留 |
| `contradiction.{side_a, side_b, sacrificed}` | `contradiction.{side_a, side_b, sacrificed}` | 保留 |
...
| `verdict.judgment` | `validation_status`（生產模式 enum）| **轉換**（見 §3.3）|
| `verdict.reason_100w` | `validation_reason` | 保留 |
| `verdict.most_confident_evidence` | `key_evidence` | 保留 |
| `verdict.next_action` | `next_step` | 保留 |
| `verdict.scores` | — | **丟棄**（見 §3.4）|
| `verdict.total_score` | — | **丟棄**（見 §3.4）|
| `self_guess.guesses` | — | 教學內部使用，不轉場（見 §3.4）|
| `self_guess.ai_checkpoints_passed` | — | 教學內部使用，不轉場 |
...
```typescript
// Worksheet 教學模式 → PainMap App 生產模式
...
```
verdict.scores       → 丟棄
verdict.total_score  → 丟棄
```

**理由**：違反 `painmap_moat_design.md M3 Anti-Score Brand` 與 `painmap_brand_system.md` L24 禁令。生產模式（PainMap App）禁止輸出 0–25 score。

**例外**：作為教學模式的內部反思鏡子，分數仍存在 Worksheet PainCard 物件中（`data_model.md` 已定義），只是**不轉場**到 Pain Entry。
...
function adaptPainCardToPainEntry(pc: PainCard): PainEntry {
  ...
  return {
    ...
    // === Explicitly dropped (Anti-Score Brand) ===
    // verdict.scores → NOT carried
    // verdict.total_score → NOT carried
    // self_guess.guesses → NOT carried
    // self_guess.ai_checkpoints_passed → NOT carried
  };
}
```
...
| 維度 | 教學模式 (Worksheet) | 生產模式 (PainMap App) |
| :--- | :--- | :--- |
| 目標 | 訓練判斷力 | 管理痛點 + 商業驗證 |
| 受眾 | 不懂 AI 的初學者 | 已通過階段一的使用者 |
| Score 顯示 | 卡 9 顯示 0–25 score（內部反思）| **完全禁止輸出 score** |
| 狀態欄位 | `status` + `verdict.judgment` | `validation_status` |
| 進度 UI | 9 卡 stepper | Pain Atlas / Disruption Mapper / Dashboard |
| 分數帶解讀 | 顯示 20–25 / 15–19 / ≤14 解讀 | 不顯示 |
| 失敗回退 | 退回上一卡 | 階段二有獨立失敗機制 |

### 4.2 Mode Switch Handover Card

當使用者點擊「匯入 PainMap App」時，系統產出一張 Handover Card 作為兩個模式的橋樑：
...
對應 `sunnydata-pain-thinking/SKILL.md` 的 Mode switch handover card：
...
### 4.4 為什麼要這個 Handover Card

1. **避免分數污染決策**：明確標示「分數不是答案」，讓使用者跨過分數直視 status
2. **建立追溯性**：未來若 sprint 失敗回頭檢查，可追到 Worksheet 原始判斷
3. **儀式感**：一個明確的轉場儀式，讓使用者感受到「我從訓練畢業，進入實戰」（白帽 #2 Accomplishment 的合法應用）
...
2. 階段一終點 = 書面真假判斷（不是付款）
3. 不可跳步（每個 exit condition 必須通過）
4. 階段一不談錢（金錢話題自動延後到階段二）
5. 0–25 score 僅限教學模式內部反思（生產模式禁止輸出）
```
...
| ✅ 沒有 score 欄位 | 符合 Anti-Score Brand |
...
### 10.2 為什麼丟棄 score 欄位這麼重要

因為一旦 score 進入 Pain Entry，PainMap App 的 UI 就會被誘惑顯示它（即便標示「僅供參考」）。最安全的方式是**根本不讓 score 進入生產模式**。
