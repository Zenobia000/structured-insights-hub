# AI Prompt Library — 7 段內建 Prompt 完整版

> **真相源**：`docs/workshop/painpoint_beginner_worksheet.md` v1.0（2026-05-01）卡片 3、4、5、6、7、8。
...
3. **不接受 AI 自行加分數**：除卡 7 第二輪追問會請 AI 整理判斷表外，其他 prompt 都不要 AI 給「真假判斷」「分數」「推薦度」。
4. **失敗 fallback 必須輕柔**：補強 prompt 不能羞辱使用者；失敗訊息以「結構化＋下一步」呈現，不出現「你錯了」「重來」。
5. **兩種輸出方式**：
...
- **前置條件**：卡 1 與卡 2 已通過 exit gate（`complaint.verbatim`、`people.list[*]` 已備齊）
...
- **前置條件**：卡 3 通過（`stuck_formula.confirmed === true`）
...
4. 助教代寫
5. ChatGPT 寫稿機（直接餵成績丟出草稿）[推測：依據常見補習圈做法]
```
...
### 3.4 反 solution mode 防護

> 「不要挑超過 1 個」 — 強制單選，避免使用者拆得不夠細又靠 AI 含糊帶過。
> 「6 個都不像 → 回卡片 3」 — 提供退場機制，不會逼 AI 硬選。
...
在主人翁身上：
- A 端（個人化）：家長要看見「我的孩子」被個別關照，要具體事件、語氣有溫度
...
| 失敗症狀 | Fallback |
| :--- | :--- |
| AI 挑了 2 個以上 | 「請只挑 1 個。如果真的覺得有兩個都很像，代表卡關公式還沒拆乾淨，請建議我退回卡片 3。」 |
| AI 直接 invent 第 7 種矛盾 | 「請只從清單的 6 種中挑，不要自創新類別。」 |
| AI 回「6 個都不像」 | 系統將自動把使用者導回卡 3，並顯示提示：「AI 認為這個卡關還沒拆乾淨，回到卡 3 把句子說得更具體吧。」 |

### 3.7 站內 LLM API payload

```json
{
  "endpoint": "/api/ai/card-5-triz-pick",
  "model": "gpt-4o-mini",
  "temperature": 0.2,
  "max_tokens": 400,
  "expected_schema": {
    "triz_id": "1 | 2 | 3 | 4 | 5 | 6 | null",
    "side_a": "string",
    "side_b": "string",
    "sacrificed": "a | b",
    "reason": "string"
  },
  "post_validation": "若 triz_id === null，前端導回 /learn/worksheet/03 並顯示退回訊息"
}
```
...
- **前置條件**：卡 1-5 都過關（系統需要 5 個欄位才能組裝 prompt）
...
> ⚠️ 這三句必須**完整保留**，不可拆任何一句。worksheet 第 343 行明文：「最重要的一句是『請不要幫我設計產品，也不要提出商業模式』」。

### 4.5 過關條件（記入 PainCard）
...
### 4.7 失敗 fallback（worksheet 卡 6「過不了」段落原文）
...
> 4 點任一不過 → 不能進這個 prompt，要先回到卡 6 補資訊重跑。
...
worksheet 第 122 行（卡 2）、第 612 行對照表、第 620-628 行：
...
- [ ] 是否與 worksheet v1.0 卡 3-8 的 prompt 文字 100% 一致？
